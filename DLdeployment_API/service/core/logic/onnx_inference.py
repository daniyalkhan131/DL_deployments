...
def emotions_detector(img_array):
    ...
    # time_elepsed_loading=time.time()-time_init
    ...
    return {'emotion': classes[result],
            ...
            'time_elapsed_preprocess': time_elapsed_preprocess}
            # 'time_elapsed_loading': time_elepsed_loading}

#majority time is taken by loading the model, so what we do we put
#loading model in main file means when page is triggered then model get
#loaded so time taken reduce